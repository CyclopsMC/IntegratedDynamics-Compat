...
public class TopSqueezerData implements IProbeInfoProvider {
    ...
    public Identifier getID() {
        return Identifier.fromNamespaceAndPath(Reference.MOD_ID, "squeezer_data");
    }

    @Override
    public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, Player player, Level world, BlockState blockState, IProbeHitData data) {
        if (world != null && blockState != null && data != null && player != null) {
            IModHelpers.get().getBlockEntityHelpers().get(world, data.getPos(), BlockEntitySqueezer.class)
                    .ifPresent(tile -> {
