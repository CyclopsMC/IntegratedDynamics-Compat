...
public class MechanicalDryingBasinDataProvider extends SqueezerDataProvider implements IComponentProvider, IServerDataProvider<BlockEntityEntity> {
    ...
    public void appendServerData(CompoundNBT tag, ServerPlayerEntity player, World world, BlockEntityEntity tileEntity) {
        ...
        if (!tile.getTankInput().isEmpty()) {
            tooltip.add(new TranslationTextComponent("gui." + Reference.MOD_ID + ".waila.fluid.in",
                    tile.getTankInput().getFluid().getDisplayName(), tile.getTankInput().getFluidAmount()));
        }
        if (!tile.getTankOutput().isEmpty()) {
            tooltip.add(new TranslationTextComponent("gui." + Reference.MOD_ID + ".waila.fluid.out",
