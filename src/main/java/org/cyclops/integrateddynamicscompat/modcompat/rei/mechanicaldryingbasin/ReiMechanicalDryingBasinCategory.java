...
public class ReiMechanicalDryingBasinCategory implements DisplayCategory<ReiMechanicalDryingBasinRecipe> {
    ...
    public ReiMechanicalDryingBasinCategory() {
        this.icon = EntryStacks.of(RegistryEntries.BLOCK_MECHANICAL_DRYING_BASIN.value());
    }
    ...
    public Component getTitle() {
        return Component.translatable(RegistryEntries.BLOCK_MECHANICAL_DRYING_BASIN.value().getDescriptionId());
    }
    ...
    public List<Widget> setupDisplay(ReiMechanicalDryingBasinRecipe display, Rectangle bounds) {
        ...
        widgets.add(Widgets.createLabel(new Point(bounds.getCenterX(), startPoint.y), JeiReiHelpers.getEnergyTextComponent(display.getRecipeDisplay().duration(), BlockMechanicalDryingBasinConfig.consumptionRate))
                .color(0xFF808080)
                .noShadow());
        widgets.add(Widgets.createLabel(new Point(bounds.getCenterX(), startPoint.y + 42), JeiReiHelpers.getDurationSecondsTextComponent(display.getRecipeDisplay().duration()))
                .color(0xFF808080)
                .noShadow());
